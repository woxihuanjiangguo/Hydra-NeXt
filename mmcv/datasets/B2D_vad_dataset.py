...
from .nuscenes_styled_eval_utils import DetectionMetrics, EvalBoxes, DetectionBox, center_distance, accumulate, \
    DetectionMetricDataList, calc_ap, calc_tp
...
class B2D_VAD_Dataset(Custom3DDataset):
    ...
    def __init__(self, cam_names,
                 ctrl_steps=4,
                 if_use_targetxy=False,
                 ego_status_feature_num=11,
                 ctrl_sample_interval=None,
                 queue_length=4, bev_size=(200, 200), overlap_test=False, with_velocity=True, sample_interval=5,
                 name_mapping=None, eval_cfg=None, map_root=None, map_file=None, past_frames=2, future_frames=6,
                 point_cloud_range=[-51.2, -51.2, -5.0, 51.2, 51.2, 3.0], polyline_points_num=20, if_shuffle=True,
                 use_future=False,
                 ctrl_sample_interval_dp=1, ctrl_steps_dp=5,
                 use_map=False,
                 *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.cam_names = cam_names
        self.ego_status_feature_num = ego_status_feature_num
        self.use_future = use_future
        self.if_use_targetxy = if_use_targetxy

        # cls control
        self.ctrl_steps = ctrl_steps
        if ctrl_sample_interval is None:
            self.ctrl_sample_interval = sample_interval
        else:
            self.ctrl_sample_interval = ctrl_sample_interval
        # dp control
        self.ctrl_sample_interval_dp = ctrl_sample_interval_dp
    ...
    def prepare_train_data(self, index):
        ...
        if self.use_future:
            index_list = list(range(index, index + 6 * self.sample_interval, self.sample_interval))

        for i in index_list:
            i = max(0, i)
            if i >= len(self.data_infos):
                i = len(self.data_infos) - 1
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            ...
            for sensor_type, cam_info in info['sensors'].items():
                if sensor_type not in self.cam_names:
                    continue
        ...
        ego_his_trajs, ego_fut_trajs, ego_fut_masks, command = self.get_ego_trajs(index, self.sample_interval,
                                                                                  self.past_frames, self.future_frames)
        ctrl_seq, ctrl_mask = self.get_control(index, self.ctrl_sample_interval, self.ctrl_steps)
        ctrl_seq_dp, ctrl_mask_dp = self.get_control(index, self.ctrl_sample_interval_dp, self.ctrl_steps_dp)
        ctrl_seq_dp_past, ctrl_mask_dp_past = self.get_control_past_one(index, self.ctrl_sample_interval_dp,
                                                                        self.ctrl_steps_dp)

        velo_seq = self.get_velo(index, self.ctrl_sample_interval)

        input_dict['ctrl_seq_dp'] = ctrl_seq_dp
        input_dict['ctrl_mask_dp'] = ctrl_mask_dp

        input_dict['ctrl_mask'] = ctrl_mask
        input_dict['ctrl_seq'] = ctrl_seq

        input_dict['ctrl_seq_dp_past'] = ctrl_seq_dp_past
        input_dict['ctrl_mask_dp_past'] = ctrl_mask_dp_past

        input_dict['velo_seq'] = velo_seq
        ...
        if prev_info['folder'] != info['folder']:
            prev_steer = 0.0
        else:
            prev_steer = prev_info['steer']
        if self.ego_status_feature_num == 11:
            input_dict['ego_status_feature'] = np.concatenate([
                input_dict['ego_vel'][0:1],
                input_dict['ego_accel'],
                np.array([prev_steer]),
                command
            ])
        elif self.ego_status_feature_num == 7:
            input_dict['ego_status_feature'] = np.concatenate([
                input_dict['ego_vel'][0:1],
                command
            ])
        assert self.ego_status_feature_num == len(input_dict['ego_status_feature'])

        input_dict['gt_brake'] = np.array(info['brake'], dtype=np.float32)
        input_dict['gt_throttle'] = np.array(info['throttle'], dtype=np.float32)
        input_dict['gt_steer'] = np.array(info['steer'], dtype=np.float32)
        theta_to_lidar = info['ego_yaw']
        if theta_to_lidar < 0:
            theta_to_lidar += 2 * np.pi

        theta_to_lidar = -theta_to_lidar + np.pi / 2
        target_xy_far = info['command_far_xy'] - info['ego_translation'][:2]
        rotation_matrix = np.array(
            [[np.cos(theta_to_lidar), -np.sin(theta_to_lidar)], [np.sin(theta_to_lidar), np.cos(theta_to_lidar)]])
        input_dict['local_command_xy'] = rotation_matrix @ target_xy_far

        if self.if_use_targetxy:
            input_dict['ego_status_feature'] = np.concatenate([
                input_dict['ego_status_feature'],
                input_dict['target_xy']
            ])


        # nn kinematic model
        ego_status = np.zeros(17)
        ego_status[0:4] = ego_his_trajs.flatten()
        ego_status[4] = info['steer']
        ego_status[5] = info['brake']
        ego_status[6] = info['throttle']
        ego_status[7:10] = info['ego_rotation_rate'].flatten()
        ego_status[10] = info['ego_yaw']
        ego_status[10:13] = info['ego_vel'].flatten()
        ego_status[13:16] = info['ego_accel'].flatten()
        input_dict['ego_status'] = ego_status
        return input_dict
    ...
    def get_map_info(self, index):
        ...
        for idx in range(len(trigger_volumes_points)):
            ...
            if mask.all():
                polylines.append(
                    LineString(np.concatenate((points_in_lidar_range, points_in_lidar_range[0:1]), axis=0)))
                gt_label = self.map_element_class[trigger_volumes_types[idx]]
                gt_labels.append(gt_label)

        gt_labels = torch.tensor(gt_labels)
        gt_bboxes = LiDARInstanceLines(polylines, fixed_num=self.polyline_points_num, patch_size=(
    ...
    def get_control(self, idx, sample_rate, future_frames):
        adj_idx_list = range(idx, idx + future_frames * sample_rate, sample_rate)
        cur_frame = self.data_infos[idx]
        full_adj_adj_mask = np.zeros(future_frames)
        ...
        for j in range(len(adj_idx_list)):
            ...
            ctrl_seq[j, 0] = adj_frame['brake']
            ctrl_seq[j, 1] = adj_frame['throttle']
            ctrl_seq[j, 2] = adj_frame['steer']
        return ctrl_seq, full_adj_adj_mask[-future_frames:].copy()

    def get_control_past_one(self, idx, sample_rate, future_frames):
        adj_idx_list = range(
            idx - 1 * sample_rate,
    ...
    def get_velo(self, idx, sample_rate):
        future_frames = self.ctrl_steps
        adj_idx_list = range(idx, idx + future_frames * sample_rate, sample_rate)
        cur_frame = self.data_infos[idx]
        velo_seq = np.zeros((
            future_frames, 1
        ...
        for j in range(len(adj_idx_list)):
            ...
            if adj_frame['folder'] != cur_frame['folder']:
                break
            velo_seq[j, 0] = adj_frame['ego_vel'][0:1]
        return velo_seq
    ...
    def get_ego_trajs(self, idx, sample_rate, past_frames, future_frames):
        ...
        return offset_track[:past_frames].copy(), offset_track[past_frames:].copy(), full_adj_adj_mask[
                                                                                     -future_frames:].copy(), command
    ...
    def get_box_attr_labels(self, idx, sample_rate, frames):
        ...
        attr_labels = np.concatenate(
            [future_track_offset.reshape(-1, frames * 2), future_mask_offset, gt_fut_goal, agent_lcf_feat,
             future_yaw_offset], axis=-1).astype(np.float32)
    ...
    def evaluate(self,
        ...
        num_valid = 0
        for res in results:
            is_valid = res['metric_results']['fut_valid_flag']
            if isinstance(is_valid, list) and isinstance(is_valid[0], torch.Tensor):
                is_valid = is_valid[0].item()
            if is_valid:
                num_valid += 1
            else:
                continue
        ...
        for k in metric_dict:
            if k == 'fut_valid_flag':
                print(f'Total: {len(results)}, valid: {num_valid}')
                continue
            metric_dict[k] = metric_dict[k] / num_valid
            print("{}:{}".format(k, metric_dict[k]))

        if self.eval_cfg.get('eval_planning_only', False):
            return None
